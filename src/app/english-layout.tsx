...
export default async function EnglishLayout({
  ...
  const hotline = config[`topbar_phone_primary_${lang}`] || HOTLINE;
  const address = config[`topbar_address_${lang}`] || SPA_ADDRESS;
  const hours = config[`topbar_hours_${lang}`] || SPA_HOURS;
  const siteName = config[`site_name_${lang}`] || config.site_name || SITE_NAME;
