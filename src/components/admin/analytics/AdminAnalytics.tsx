    ...
    <div className="space-y-8">
      <section className="admin-panel px-6 py-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-white">Analytics theo ngay</h1>
        ...
        <Card className="border-white/5">
          ...
          <CardContent>
            <p className="text-2xl font-semibold text-white">
              {(data?.comparison.revenue.current || 0).toLocaleString()} d
            </p>
            <p className="text-xs text-white/50">
              Ky truoc: {(data?.comparison.revenue.previous || 0).toLocaleString()} d
            </p>
          </CardContent>
        </Card>
        <Card className="border-white/5">
          ...
          <CardContent>
            ...
            <p className="text-xs text-white/50">Ky truoc: {data?.comparison.bookings.previous || 0}</p>
          </CardContent>
        </Card>
        <Card className="border-white/5">
          <CardHeader className="flex flex-row items-center justify-between">
          ...
          <CardContent>
            ...
            <p className="text-xs text-white/50">Ky truoc: {data?.comparison.users.previous || 0}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="border-white/5 lg:col-span-2">
          ...
          <CardContent>
            <LineChart labels={data?.labels || []} data={data?.series.revenue || []} label="Revenue" />
          </CardContent>
        </Card>
        <Card className="border-white/5">
          <CardHeader>
